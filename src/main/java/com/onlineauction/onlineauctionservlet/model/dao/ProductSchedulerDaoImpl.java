...
public class ProductSchedulerDaoImpl implements ProductSchedulerDao {
    ...
    public int scheduleAuction(ProductForAuction productAuction) {
        ...
        if (checkIfBidScheduled(productAuction.getProductId())) {
            ...
            try {
                ps = conn.prepareStatement("insert into ProductBid(MinBidValue,BidStartDate,BidEndDate,ProductID,Status) values(?,?,?,?,?)");
    ...
    public List<Product> getProductList(int sellerId) {
        ...
        ResultSet resultset;
        try {
            ps = conn.prepareStatement("select ProductID,ProductName from Product where SellerID=?");
    ...
    public boolean checkIfBidScheduled(int productId) {
        ...
        ResultSet resultset;
        try {
            String query = "select * from ProductBid where ProductID=?";
